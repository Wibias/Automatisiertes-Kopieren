...
public class ExcelHelper
{
    ...
    private static bool AreNamesSimilar(string name1, string name2)
    {
        ...
        return distance is <= threshold and > 0;
    }
    ...
    {
        string? parsedBirthDate = null; // Declare and initialize it here
        ...
        try
        {
            ...
            try
            {
                ...
                for (var row = 7; row <= 31; row++)
                {
                    ...
                    {
                        // Names are not exactly the same; check if they are very similar
                        if (!AreNamesSimilar(excelFirstName, directoryFirstName) &&
    ...
    public async Task<bool> SelectHeutigesDatumEntwicklungsBerichtAsync(object sender, string group)
    {
        ...
        {
            LogAndShowMessage(ex.Message, "Fehler beim Aktualisieren der Excel-Datei.");
            return false;
        }
    }
}